...
def connect_master(**kwargs):
  ...
  if(ctx._local):
    ...
    ctx.logger.info('got deployment proxy URL: {0}'.format(ctx.target.instance.runtime_properties['kubernetes_info']['url']))
    ...
    ctx.logger.info('master_ip: {0} master_port: {1}'.format(
      ctx.source.instance.runtime_properties['master_ip'],
      ctx.source.instance.runtime_properties['master_port']))
  else:
...
def kube_run_expose(**kwargs):
  ...
  env['user']=ctx.node.properties['ssh_username']
  env['key_filename']=ctx.node.properties['ssh_keyfilename']
  ctx.logger.info('the env at this point: \n{0}'.format(env))
